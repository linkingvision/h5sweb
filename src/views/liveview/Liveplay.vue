  ...
  <div class="liveplay h5container">
    ...
    <div :id="videonameid" class="">
      ...
      <input
        type="button"
        :value="valuebutton"
        @click="Bitstream($event.target.value)"
        :id="inputid"
        class=""
      />
    </div>
    ...
    <span class="" :id="spanqualityid">
      ...
      >
        <span :id="elqualityid" class="el-dropdown-link">
          <i class="el-icon-video-camera el-icon--left"></i
          >{{ $t("message.live.Quality") }}
        </span>
        <el-dropdown-menu slot="dropdown">
          <template>
            <el-dropdown-item
              v-for="(it, index) in qualitylist"
              :key="index"
              :command="it"
              >{{ it.strName }}</el-dropdown-item
            >
          </template>
        </el-dropdown-menu>
      </el-dropdown>
      ...
      <div class="information_con">
        <div class="information1">
          ...
          <div
            ...
            v-for="(a, index) in informationVideo"
            :key="index"
          >
        ...
        <div class="information1">
          ...
          <div
            class="information_content"
            v-for="(a, index) in informationAudio"
            :key="index"
          >
            <div class="information_content_left">{{ a.name }}</div>
            <div class="information_content_right">{{ a.data }}</div>
    ...
    <div class="liveplay_butt">
      ...
      <button
        class="info_id_show iconfont"
        :class="Audioslider == 0 ? 'icon-jingyin' : 'icon-mn_shengyin_fill'"
        @click="Audiosetup"
      ></button>
      <button
        class="info_id_show iconfont icon-liuliang"
        @click="Information"
      ></button>
      <button
        class="iconfont"
        :class="Shoutwheatclass"
        @click="Shoutwheat"
      ></button>
      ...
      <button
        class="iconfont icon-videofill"
        @click="DoManualRecordStart"
      ></button>
      <button
        class="iconfont icon-radioboxfill"
        @click="DoManualRecordStop"
      ></button>
      <button
        class="ptz_id_show iconfont icon-yuntai"
        @click="PtzControlShow"
      ></button>
      <button class="iconfont icon-full" @click="FullScreen"></button>
      <button
        class="iconfont icon-roundclosefill"
        @click="CloseVideo('close')"
      ></button>
    </div>
    <div class="liveplay_ptz" style="display:none padding:0px">
      <div class="flex_content">
        <div class="content_zoom">
          <div class="key_zoom">
            <div class="key_flex">
              <div
                class="key_but"
                @mousedown="PtzActionUpleft($event)"
                @mouseup="PtzActionStop($event)"
                @touchstart="PtzActionUpleft($event)"
                @touchend="PtzActionStop($event)"
              ></div>
              <div
                class="key_but"
                @mousedown="PtzActionUp($event)"
                @mouseup="PtzActionStop($event)"
                @touchstart="PtzActionUp($event)"
                @touchend="PtzActionStop($event)"
              ></div>
              <div
                class="key_but"
                @mousedown="PtzActionUpright($event)"
                @mouseup="PtzActionStop($event)"
                @touchstart="PtzActionUpright($event)"
                @touchend="PtzActionStop($event)"
              ></div>
              <div
                class="key_but"
                @mousedown="PtzActionLeft($event)"
                @mouseup="PtzActionStop($event)"
                @touchstart="PtzActionLeft($event)"
                @touchend="PtzActionStop($event)"
              ></div>
              <div class="key_but"></div>
              <div
                class="key_but"
                @mousedown="PtzActionRight($event)"
                @mouseup="PtzActionStop($event)"
                @touchstart="PtzActionRight($event)"
                @touchend="PtzActionStop($event)"
              ></div>
              <div
                class="key_but"
                @mousedown="PtzActionDownleft($event)"
                @mouseup="PtzActionStop($event)"
                @touchstart="PtzActionDownleft($event)"
                @touchend="PtzActionStop($event)"
              ></div>
              <div
                class="key_but"
                @mousedown="PtzActionDown($event)"
                @mouseup="PtzActionStop($event)"
                @touchstart="PtzActionDown($event)"
                @touchend="PtzActionStop($event)"
              ></div>
              <div
                class="key_but"
                @mousedown="PtzActionDownright($event)"
                @mouseup="PtzActionStop($event)"
                @touchstart="PtzActionDownright($event)"
                @touchend="PtzActionStop($event)"
              ></div>
            </div>
          </div>
          <div class="zoom">
            <el-tooltip
              :enterable="false"
              :content="content.focusing"
              placement="top"
              effect="light"
            >
              <button
                class="iconfont icon-add-focus zoom_add"
                @mousedown="PtzActionZoomIn($event)"
                @mouseup="PtzActionStop($event)"
                @touchstart="PtzActionZoomIn($event)"
                @touchend="PtzActionStop($event)"
              ></button>
            </el-tooltip>
            <el-tooltip
            ...
            >
              <button
                class="iconfont icon-reduce-focus zoom_add"
                @mousedown="PtzActionZoomOut($event)"
                @mouseup="PtzActionStop($event)"
                @touchstart="PtzActionZoomOut($event)"
                @touchend="PtzActionStop($event)"
              ></button>
            </el-tooltip>
          </div>
        ...
        <div class="zoom_g">
          ...
          >
            <button
              class="iconfont icon-jujiao2 zoom_add"
              @mousedown="PtzActionfocusing($event)"
              @mouseup="PtzActionStop($event)"
              @touchstart="PtzActionfocusing($event)"
              @touchend="PtzActionStop($event)"
            ></button>
          </el-tooltip>
          <el-tooltip
          ...
          >
            <button
              class="iconfont icon-jujiao1 zoom_add"
              @mousedown="PtzActionfocusings($event)"
              @mouseup="PtzActionStop($event)"
              @touchstart="PtzActionfocusings($event)"
              @touchend="PtzActionStop($event)"
            ></button>
          </el-tooltip>
          ...
          <el-tooltip
            :enterable="false"
            :content="content.aperture"
            placement="top"
            effect="light"
          >
            <button
              class="iconfont icon-guangquanjia zoom_add"
              @mousedown="PtzActionaperture($event)"
              @mouseup="PtzActionStop($event)"
              @touchstart="PtzActionaperture($event)"
              @touchend="PtzActionStop($event)"
            ></button>
          </el-tooltip>
          <el-tooltip
          ...
          >
            <button
              class="iconfont icon-guangquanjian zoom_add"
              @mousedown="PtzActionaperturej"
              @mouseup="PtzActionStop"
              @touchstart="PtzActionaperturej"
              @touchend="PtzActionStop"
            ></button>
          </el-tooltip>
          <!-- 后四 -->
          <el-tooltip
            :enterable="false"
            :content="content.LightON"
            placement="top"
            effect="light"
          >
            <button
              class="iconfont icon-light-open zoom_add"
              @mousedown="PtzActionlighton($event)"
              @mouseup="PtzActionStop($event)"
              @touchstart="PtzActionfocusing($event)"
              @touchend="PtzActionStop($event)"
            ></button>
          ...
          <el-tooltip
            :enterable="false"
            :content="content.LightOFF"
            placement="top"
            effect="light"
          >
            <button
              class="iconfont icon-light-close zoom_add"
              @mousedown="PtzActionlightoff($event)"
              @mouseup="PtzActionStop($event)"
              @touchstart="PtzActionfocusings($event)"
              @touchend="PtzActionStop($event)"
            ></button>
          ...
          <el-tooltip
            :enterable="false"
            :content="content.WiperON"
            placement="top"
            effect="light"
          >
            <button
              class="iconfont icon-kaiyushua zoom_add"
              @mousedown="PtzActionwiperon($event)"
              @mouseup="PtzActionStop($event)"
              @touchstart="PtzActionaperture($event)"
              @touchend="PtzActionStop($event)"
            ></button>
          ...
          <el-tooltip
            :enterable="false"
            :content="content.WiperOFF"
            placement="top"
            effect="light"
          >
            <button
              class="iconfont icon-guanyushua zoom_add"
              @mousedown="PtzActionwiperoff($event)"
              @mouseup="PtzActionStop($event)"
              @touchstart="PtzActionaperture($event)"
              @touchend="PtzActionStop($event)"
            ></button>
                ...
                <el-card>
                  <div class="preset_bgc">
                    ...
                    <button
                      ...
                      @click="preset_Jump(Pre.strToken)"
                    ></button>
                    <button
                      type="button"
                      class="iconfont icon-icon-test1"
                      @click="preset_set(Pre.strToken, $event)"
                    ></button>
                  </div>
                </el-card>
              </el-timeline-item>
...
export default {
  ...
  watch: {
    listData(token) {
      ...
      setTimeout(() => {
        ...
        console.log(this.h5id,token.vid);
  ...
  methods: {
    ...
    Informationdate() {
      ...
      var url =
        root +
        "/api/v1/GetVidStreamStatus?token=" +
        this.tokenshou +
        "&stream=" +
        this.streamprofileInformation +
        "&session=" +
...
=======
    ...
    Bitstream(event) {
      console.log(event, this.$t("message.live.substream"));
      console.log(this.$t("message.live.mainstream"));
      // return false
      if (event === this.$t("message.live.substream")) {
        var streamprofile = "sub";
        var token = this.inputtoken;
    ...
    Gettranscod() {
      var url =
        this.$store.state.IPPORT +
        "/api/v1/GetTransProfile?session=" +
        this.$store.state.token;
      this.$http
        .get(url)
        .then((result) => {
    ...
    handleCommand(command) {
      ...
      this.videoname = name;
      var streamprofile = command.strToken;
    //   console.log(token, streamprofile, name);
      this.PlayVideo(token, streamprofile, "", name);
    },
    //播放
    PlayVideo(token, streamprofile, label, name) {
      ...
      .content_zoom {
        ...
        .key_zoom {
          ...
          .key_flex {
            ...
            justify-content: space-between;
            margin-left: 5%;
            background: url("~@/views/liveview/imgs/liveview_ptzbutton.png")
              no-repeat center;
            /* margin-bottom: 20px; */
            .key_but {
              width: 33.33%;
